defmodule Deputy.UtilityTest do
  ...
  describe "get_time/1" do
    test "retrieves current system time", %{client: client} do
      ...
      |> expect(:request, fn %Deputy.HTTPClient.Request{
                               method: :get,
                               url: "https://test.deputy.com/api/v1/time"
                             } ->
        {:ok, response_body}
      end)
  ...
  describe "create_memo/2" do
    test "creates a memo", %{client: client} do
      ...
      |> expect(:request, fn %Deputy.HTTPClient.Request{
                               method: :put,
                               url: "https://test.deputy.com/api/v1/supervise/memo"
                             } = req ->
  ...
  describe "add_webhook/2" do
    test "adds a webhook", %{client: client} do
      ...
      |> expect(:request, fn %Deputy.HTTPClient.Request{
                               method: :post,
                               url: "https://test.deputy.com/api/v1/resource/Webhook"
                             } = req ->
        assert req.body == attrs
  ...
  describe "who_am_i!/1" do
    test "returns unwrapped user info", %{client: client} do
      ...
      Deputy.HTTPClient.Mock
      |> expect(:request, fn %Deputy.HTTPClient.Request{url: "https://test.deputy.com/api/v1/me"} ->
        {:ok, response_body}
      end)
